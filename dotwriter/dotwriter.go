...
func (ctx *plotCtx) Deeper() *plotCtx {
	return &plotCtx{
		...
		level:       ctx.level - 1,
		hideNoFiles: ctx.hideNoFiles,
	}
}
func newPlotContext(level int, hideNoFiles bool) *plotCtx {
	return &plotCtx{
		level:       level,
		nodeFlags:   make(map[string]bool),
		edgeFlags:   make(map[string]bool),
		hideNoFiles: hideNoFiles,
	}
}
...
func (dw *DotWriter) PlotGraph(root IDotNode) {
	...
	dw.plotNode(newPlotContext(dw.MaxDepth, dw.HideNoFiles), root)
	dw.printLine("}")
}
...
func (dw *DotWriter) plotNode(ctx *plotCtx, node IDotNode) {
	...
	for _, s := range dw.getDependency(node) {
		if s, ok := s.(interface{ HasFiles() bool }); ok && !s.HasFiles() {
			continue
		}
